...
async def render_edit_todo_page(db: db_dependency, request: Request, todo_id: int):
    try:
        user = await get_current_user(request.cookies.get("access_token"))
        if user is None:
            return redirect_to_login()
...
async def create_to_do(user: user_dependency, db: db_dependency,
...
async def update_to_do(user: user_dependency,
                       db: db_dependency,
                       todo_request: TodoRequest,
...
async def delete_to_do(user: user_dependency,
                       db: db_dependency,
                       todo_id: int = Path(gt=0)):
    if user is None:
        raise HTTPException(status_code=401, detail="Autentication failed")
